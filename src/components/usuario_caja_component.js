...
const RegistroUsuario = ({ setCurrentComponent }) => {
  ...
  const cargarRoles = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/api/rol/roles-caja`);
  ...
  const cargarUsuarios = async () => {
    try {
      const response = await axios.get(`${config.API_BASE_URL}/api/usuario/get-users-by-idcaho?idCaho=${idCaho}`);
      if (response.data && Array.isArray(response.data)) {
      ...
      try {
        ...
        if (editing) {
          ...
          response = await axios.post(
            `${config.API_BASE_URL}/api/usuario/save`,
            usuarioDTO,
            {
              headers: {
                'Content-Type': 'application/json'
