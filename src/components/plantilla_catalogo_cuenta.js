...
const PlantillaCatalogoCuenta = ({ setCurrentComponent }) => {
  ...
  const [cuentasIniciales, setCuentasIniciales] = useState([]);
...
*/
  ...
  // Método para cargar cuentas de una plantilla específica
  const cargarCuentasPlantilla = async (idPPlanContable) => {
    try {
      const response = await axios.get(`${baseUrl}/api/plantillas-plan-catalogo/plantilla/${idPPlanContable}`, {
        headers: token ? { Authorization: `${token}` } : {}
      });

      // Transformar los datos de la plantilla al formato esperado por la tabla
      const cuentasTransformadas = response.data.map(cuentaPlantilla => ({
        idPPlanCatalogo: cuentaPlantilla.idPPlanCatalogo,
        cuenta: cuentaPlantilla.cuenta,
        descripcion: cuentaPlantilla.descripcion,
        auxiliarTipo: auxiliarTipo.find(aux => aux.idTaux === cuentaPlantilla.idTaux) || null,
        movimiento: cuentaPlantilla.movimiento,
        tplan: cuentaPlantilla.tplan
      }));
      
      setCuentas(cuentasTransformadas);
    } catch (error) {
      show_alerta('Error al cargar las cuentas de la plantilla', 'error');
    }
  };
  ...
  const handlePlantillaChange = async (e) => {
    ...
    handleClean();
    if (selectedValue === '') {
      // Si se selecciona "Seleccione una plantilla...", restaurar las cuentas iniciales
      setCuentas(cuentasIniciales);
    } else {
      // Cargar las cuentas de la plantilla seleccionada
      await cargarCuentasPlantilla(selectedValue);
    }
    
  };

  // Método para validar el código contable
  const validarCodigoContable = async (idPPlanContable, cuenta) => {
    try {
      const response = await axios.get(baseUrl + '/api/plantillas-plan-catalogo/validar-cuenta', {
        headers: token ? { Authorization: `${token}` } : {},
        params: {
          idPPlanContable: idPPlanContable,
          cuenta: cuenta
        }
      });
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedRows(filteredCuentas.map(item => item.idPPlanCatalogo));
    }
  ...
  const loadDataForEdit = (cuentaItem) => {
    ...
    setEditingId(cuentaItem.idPPlanCatalogo);
  ...
  const handleDelete = async (id) => {
    ...
    console.log(id)
    try {
      await axios.delete(`${baseUrl}/api/plantillas-plan-catalogo/eliminar`, {
        headers: {
          Authorization: `${token}`
        },
        params: {
      ...
      setCuentas(cuentas.filter(item => item.idPPlanCatalogo !== id));
    } catch (error) {
  ...
  const handleDeleteMultiple = async () => {
    ...
    try {
      ...
      setCuentas(cuentas.filter(item => !selectedRows.includes(item.idPPlanCatalogo)));
      setSelectedRows([]);
      setSelectAll(false);
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!cuenta || !descripcion || !selectedPlantilla) {
      show_alerta('Los campos Plan Contable, Cuenta y Descripción son obligatorios', 'error');
      return false;
    }
    ...
    try {
      ...
      if (editingId) {
        formData = {
            idPPlanCatalogo: editingId,
            descripcion: descripcion
        };

        // Modificar cuenta existente
        response = await axios.put(
          `${baseUrl}/api/plantillas-plan-catalogo/update`,
          formData,
          {
            headers: {
              Authorization: `${token}`,
        ...
        //CAMBIAR *********IMPORTANTE****************
        const resCodigoContable = await validarCodigoContable(selectedPlantilla, cuenta);
        ...
        response = await axios.post(
          `${baseUrl}/api/plantillas-plan-catalogo/save`,
          formData,
          {
            headers: {
              Authorization: `${token}`,
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (editingId) {
          setCuentas(cuentas.map(item =>
            item.idPPlanCatalogo === editingId ? { ...item, ...formData } : item
          ));
        } else {
            console.log("formData",formData);
            console.log("response.data save",response.data);
          // Agregar nueva cuenta a la tabla
          setCuentas([...cuentas, { ...response.data, idPPlanCatalogo: response.data.idPPlanCatalogo }]);
        }
          ...
          <tbody>
            {filteredCuentas.length > 0 ? (
              filteredCuentas.map(cuentaItem => {
                ...
                return (
                  <tr key={cuentaItem.idPPlanCatalogo}>
                    <td>
                        <input
                            type="checkbox"
                            disabled={isBloqueado}
                            checked={selectedRows.includes(cuentaItem.idPPlanCatalogo)}
                            onChange={() => handleRowSelect(cuentaItem.idPPlanCatalogo)}
                        />
                    </td>
                    ...
                    <td>
                        <div className="action-buttons">
                            <button
                            className={`${!isBloqueado ? 'edit-btn' : 'bloquear'}`}
                            onClick={() => loadDataForEdit(cuentaItem)}
                            >
                            <FaEdit />
                            </button>
                            <button
                            className={`${!isBloqueado ? 'delete-btn' : 'bloquear'}`}
                            onClick={() => handleDelete(cuentaItem.idPPlanCatalogo)}
                            >
                            <FaTrash />
                            </button>
                        </div>
...
export default PlantillaCatalogoCuenta;
